...
const Month = ({ month }) => {
  let monthFirstDayDate = new Date(buildDate(month)) //builds date to be accurate

  let numberOfDays = new Date(2018, month, 0).getDate(); //gets number of days in
  ...
  while(day < 31){
    ...
    for(let i = index; i <= 6; i++){
      if(day > numberOfDays){
        ...
        const dayLink = `/month/${month}/day/${day}`
        week[i] = <Link to={dayLink}>
        <Day day = {day} today = {today == day && mm == month} weekend = {i == 0 || i == 6}/>
      </Link>
      }
      day = day + 1;
    }
  ...
  const MonthTitle = () => (<Text text = {months[month - 1]} />)
